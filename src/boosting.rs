...
impl<'a> TreeBuilder<'a> {
    ...
    fn find_best_split(&self, sample_indices: &[usize]) -> Result<Option<BestSplit>> {
        ...
        // Only return split if gain is significant (use default min gain of 0.0)
        let min_split_gain = 0.0; // TODO: Add min_split_gain to Config
    ...
    fn calculate_split_gain(&self, left_indices: &[usize], right_indices: &[usize], parent_indices: &[usize]) -> Result<f64> {
        let parent_gain = self.calculate_node_gain(parent_indices)?;
        let left_gain = self.calculate_node_gain(left_indices)?;
        let right_gain = self.calculate_node_gain(right_indices)?;
        
        // Split gain = parent_gain - (left_gain + right_gain)
        Ok(parent_gain - left_gain - right_gain)
    }
    
    /// Calculate gain for a node (negative variance, higher is better)
    fn calculate_node_gain(&self, sample_indices: &[usize]) -> Result<f64> {
        ...
        // Gain = -0.5 * (sum_gradients^2) / (sum_hessians + lambda_l2)
        let regularized_hessian = sum_hessians + self.config.lambda_l2 as f64;
        let gain = -0.5 * (sum_gradients * sum_gradients) / regularized_hessian;
        
        Ok(gain)
    }
    
    /// Calculate optimal leaf value for samples
