...
/// Default filename for saved model files.
...
pub mod utils {
            ...
            match key.as_str() {
                "num_iterations" => {
                    let val = value.parse::<usize>().map_err(|_| {
                        LightGBMError::config(format!("Invalid num_iterations: {}", value))
                    })?;
                    builder.config.num_iterations = val;
                }
                "learning_rate" => {
                    let val = value.parse::<f64>().map_err(|_| {
                        LightGBMError::config(format!("Invalid learning_rate: {}", value))
                    })?;
                    builder.config.learning_rate = val;
                }
                "num_leaves" => {
                    let val = value.parse::<usize>().map_err(|_| {
                        LightGBMError::config(format!("Invalid num_leaves: {}", value))
                    })?;
    ...
    pub fn config_to_map(config: &Config) -> HashMap<String, String> {
        ...
        map.insert(
            ...
            config.feature_fraction.to_string(),
        );
        map.insert(
            "bagging_fraction".to_string(),
            config.bagging_fraction.to_string(),
        );
        ...
        map.insert(
            "early_stopping_tolerance".to_string(),
            config.early_stopping_tolerance.to_string(),
        );
