...
impl NumericalBinner {
    ...
    fn fit_quantile(&self, values: &[f32]) -> Result<(BinMapper, MissingType)> {
        ...
        if valid_values.is_empty() {
            return Err(LightGBMError::dataset("No valid values for quantile binning"));
        }
    ...
    fn fit_adaptive(&self, values: &[f32]) -> Result<(BinMapper, MissingType)> {
        ...
        if valid_values.is_empty() {
            return Err(LightGBMError::dataset("No valid values for adaptive binning"));
        }
        ...
        while bin_upper_bounds.len() < self.config.max_bins - 1 && current_start < valid_values.len() {
    ...
    fn calculate_statistics(&self, values: &[f32]) -> NumericalBinningStats {
        ...
        let max_value = valid_values.iter().fold(f32::NEG_INFINITY, |a, &b| a.max(b)) as f64;
        ...
        let num_unique = sorted_values.iter()
            .enumerate()
...
pub mod utils {
    ...
    pub fn calculate_information_gain(
        ...
        let left_values: Vec<f32> = values.iter()
            .copied()
            .filter(|&x| x <= split_point)
            .collect();
        
        let right_values: Vec<f32> = values.iter()
            .copied()
            .filter(|&x| x > split_point)
            .collect();
        ...
        match targets {
            Some(target_values) => {
                ...
                let left_targets: Vec<f32> = values.iter()
                    .zip(target_values)
                    .filter(|(&x, _)| x <= split_point)
                    .map(|(_, &y)| y)
                    .collect();
                
                let right_targets: Vec<f32> = values.iter()
                    .zip(target_values)
                    .filter(|(&x, _)| x > split_point)
                    .map(|(_, &y)| y)
                    .collect();
        ...
        while splits.len() < max_splits && !segments.is_empty() {
            ...
            for (seg_idx, &(start, end)) in segments.iter().enumerate() {
                ...
                for split_idx in min_samples_per_bin..(segment_values.len() - min_samples_per_bin) {
                    ...
                    let gain = calculate_information_gain(segment_values, split_value, segment_targets);
