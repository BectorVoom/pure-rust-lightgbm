...
impl CsvLoader {
    ...
    fn convert_to_dataset(
        ...
        // Separate feature columns (exclude target and weight columns)
        ...
        for (row_idx, record) in records.iter().enumerate() {
            ...
            labels[row_idx] = self.parse_numeric_value(label_str, row_idx, target_col_idx, true)?;
            ...
            if let (Some(ref mut weights_array), Some(weight_idx)) = (weights.as_mut(), weight_col_idx) {
                ...
                weights_array[row_idx] = self.parse_numeric_value(weight_str, row_idx, weight_idx, false)?;
            }
