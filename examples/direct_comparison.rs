...
fn create_test_datasets() -> Result<HashMap<&'static str, (Array2<f32>, Array1<f32>)>> {
    ...
    let features1 = Array2::from_shape_vec(
        (20, 3),
        vec![
            1.0, 2.0, 0.5, 2.0, 1.0, 1.0, 3.0, 0.5, 2.0, 0.5, 3.0, 0.2, 2.5, 1.5, 1.2, 1.5, 2.5,
            0.8, 3.2, 0.8, 1.8, 0.8, 2.8, 0.5, 2.0, 2.0, 1.0, 1.0, 1.0, 1.5, 3.5, 1.2, 2.2, 1.2,
            3.2, 0.3, 2.8, 1.8, 1.5, 1.8, 2.2, 1.1, 3.0, 1.0, 2.0, 1.0, 3.0, 0.8, 2.2, 2.3, 1.3,
            2.3, 1.7, 0.9, 3.3, 0.7, 1.9, 0.7, 2.7, 0.4,
        ],
    )
    ...
    let features2 = Array2::from_shape_vec(
        (20, 3),
        vec![
            10.0, 20.0, 5.0, 20.0, 10.0, 10.0, 30.0, 5.0, 20.0, 5.0, 30.0, 2.0, 25.0, 15.0, 12.0,
            15.0, 25.0, 8.0, 32.0, 8.0, 18.0, 8.0, 28.0, 5.0, 20.0, 20.0, 10.0, 10.0, 10.0, 15.0,
            35.0, 12.0, 22.0, 12.0, 32.0, 3.0, 28.0, 18.0, 15.0, 18.0, 22.0, 11.0, 30.0, 10.0,
            20.0, 10.0, 30.0, 8.0, 22.0, 23.0, 13.0, 23.0, 17.0, 9.0, 33.0, 7.0, 19.0, 7.0, 27.0,
            4.0,
        ],
    )
    .map_err(|e| LightGBMError::dataset(format!("Failed to create feature array: {}", e)))?;
    ...
    let features3 = Array2::from_shape_vec(
        (20, 3),
        vec![
            0.1, 0.2, 0.05, 0.2, 0.1, 0.1, 0.3, 0.05, 0.2, 0.05, 0.3, 0.02, 0.25, 0.15, 0.12, 0.15,
            0.25, 0.08, 0.32, 0.08, 0.18, 0.08, 0.28, 0.05, 0.2, 0.2, 0.1, 0.1, 0.1, 0.15, 0.35,
            0.12, 0.22, 0.12, 0.32, 0.03, 0.28, 0.18, 0.15, 0.18, 0.22, 0.11, 0.3, 0.1, 0.2, 0.1,
            0.3, 0.08, 0.22, 0.23, 0.13, 0.23, 0.17, 0.09, 0.33, 0.07, 0.19, 0.07, 0.27, 0.04,
        ],
    )
    .map_err(|e| LightGBMError::dataset(format!("Failed to create feature array: {}", e)))?;
...
fn train_rust_model(features: &Array2<f32>, labels: &Array1<f32>) -> Result<Vec<f32>> {
    ...
    let config = ConfigBuilder::new()
        ...
        .num_leaves(7)
        .min_data_in_leaf(2)
        .lambda_l1(0.0) // Python default
        .lambda_l2(1.0)
        .random_seed(42) // Python seed
        .build()?;
    ...
    let training_predictions = regressor.predict(&features)?;
    ...
    let test_predictions = regressor.predict(&test_features)?;
        ...
        if let Some(python_data) = python_results.get(dataset_name) {
            ...
            for i in 0..20.min(rust_predictions.len()) {
                ...
                if i < 3 {
                    ...
                    println!(
                        "  Sample {}: Rust={:.6}, Python={:.6}, diff={:.2e}",
