...
fn test_gbdt_tree_learner_integration() {
    ...
    assert_eq!(
        parallel_gbdt.config().tree_learner,
...
fn test_objective_function_integration() {
    ...
    for objective in objectives {
        let config = match objective {
            ObjectiveType::Multiclass => ConfigBuilder::new()
                .objective(objective)
                .num_class(3)
                .build()
                .unwrap(),
            _ => ConfigBuilder::new().objective(objective).build().unwrap(),
        };
...
fn test_prediction_pipeline_integration() {
    ...
    let pred_configs = vec![
        ...
        PredictionConfig::new().with_predict_contrib(true),
        PredictionConfig::new()
    ...
    for pred_config in pred_configs {
        ...
        assert_eq!(
            predictor.config().predict_leaf_index,
            pred_config.predict_leaf_index
        );
        assert_eq!(
            predictor.config().predict_contrib,
            pred_config.predict_contrib
        );
    }
...
fn test_error_propagation_integration() {
    ...
    let dataset =
        Dataset::new(valid_features.clone(), valid_labels, None, None, None, None).unwrap();
