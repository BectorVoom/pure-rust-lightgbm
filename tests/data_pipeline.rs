...
fn test_pipeline_with_missing_values() {
    ...
    let dataset_config = DatasetConfig::new().with_max_bin(256).with_two_round(true); // Enable two-round for missing values
...
fn test_performance_pipeline() {
    ...
    let sizes = vec![(100, 10), (500, 20), (1000, 50), (2000, 100)];
    ...
    for (num_samples, num_features) in sizes {
        ...
        let dataset =
            Dataset::new(features.clone(), labels.clone(), None, None, None, None).unwrap();
...
fn test_error_recovery_pipeline() {
    ...
    let invalid_dataset = Dataset::new(features, labels, None, None, None, None);
    ...
    let invalid_dataset_config = DatasetConfig::new().with_max_bin(1); // Too small
...
fn test_memory_efficient_pipeline() {
    ...
    println!(
        "  Memory usage: start={}MB, peak={}MB, final={}MB",
...
fn test_concurrent_pipeline() {
    ...
    for handle in handles {
        let (thread_id, stats, validation_result, feature_types, training_result) =
            handle.join().unwrap();
