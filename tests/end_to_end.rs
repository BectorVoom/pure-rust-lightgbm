...
fn test_complete_binary_classification_workflow() {
    ...
    match classifier.fit(&train_dataset) {
        Ok(_) => {
            ...
            // Predictions should be identical
            for (orig, loaded) in class_predictions
                .iter()
                .zip(loaded_class_predictions.iter())
            {
...
fn test_cross_validation_workflow() {
    ...
    let cv_result = cross_validate(&dataset, &model_config, &cv_config, &["rmse", "mae"]);
...
fn test_early_stopping_workflow() {
    ...
    let train_dataset = Dataset::new(train_features, train_labels, None, None, None, None).unwrap();
    ...
    match regressor.fit(&train_dataset) {
        Ok(_) => {
            ...
            println!(
                "  ✓ Training stopped early at iteration {}",
                actual_iterations
            );
            ...
            println!(
                "  ✓ Training history available with {} iterations",
...
fn test_hyperparameter_optimization_workflow() {
    ...
    let opt_result = optimize_hyperparameters(&dataset, &param_space, &opt_config);
