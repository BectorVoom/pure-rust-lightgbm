...
fn test_performance_scalability_workflow() {
    ...
    let sizes = vec![(100, 5), (500, 10), (1000, 20), (2000, 50)];
...
fn run_regression_workflow(temp_dir: &TempDir) -> Result<RegressionMetrics> {
    ...
    if train_stats.num_features != test_stats.num_features {
        return Err(LightGBMError::data_dimension_mismatch(format!(
            "Feature count mismatch between train and test: expected {}, got {}",
...
fn run_classification_workflow(temp_dir: &TempDir) -> Result<ClassificationMetrics> {
    ...
    if class_counts[0] == 0 || class_counts[1] == 0 {
        return Err(LightGBMError::data_validation(
            "Imbalanced dataset with missing class",
        ));
    }
...
fn test_mixed_datatypes_pipeline(temp_dir: &TempDir) -> Result<()> {
    ...
    let config = DatasetConfig::new().with_categorical_features(categorical_indices);

    let dataset = DatasetFactory::from_arrays(features, labels, None, config)?;
...
fn test_feature_importance_workflow(temp_dir: &TempDir) -> Result<()> {
    ...
    let dataset = Dataset::new(features.clone(), labels, None, None, feature_names, None)?;
...
fn calculate_mape(
    predictions: &ndarray::ArrayView1<f32>,
    true_values: &ndarray::ArrayView1<f32>,
) -> f32 {
    let mut sum = 0.0;
    let mut count = 0;
...
fn calculate_log_loss(
    probabilities: &ndarray::ArrayView1<f32>,
    labels: &ndarray::ArrayView1<f32>,
) -> f32 {
    let mut sum = 0.0;
